		...
		if filename in files:
			...
			for i in contents:
				...
				if curses.getsyx()[0] == curses.LINES - 1: #y handling
					myPad.posy += 1
					pad.resize(pad.getmaxyx()[0] + myPad.posy, pad.getmaxyx()[1])
				pad.addstr(i)
				pad.refresh(myPad.posy, myPad.posx, 0, 0, curses.LINES - 1, curses.COLS - 1)
			myPad.posy = 0
			pad.refresh(myPad.posy, myPad.posx, 0, 0, curses.LINES - 1, curses.COLS - 1)
			pad.move(0, 0)
		else:
...
def save_close(pad, myPad, filename):
	contents = []
